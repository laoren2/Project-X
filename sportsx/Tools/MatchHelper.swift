...
struct RunningPathPointTool {
    ...
    static func computeTrainingSamplePoints(pathData: [RunningTrainingPathPoint]) -> [RunningTrainingSamplePathPoint] {
        guard !pathData.isEmpty else { return [] }
        if pathData.count <= 80 {
            // 当点数不超过80时，直接将每个点转换为区间相同的TestSamplePathPoint
            return pathData.map { p in
                RunningTrainingSamplePathPoint(
                    speed_avg: max(3.6 * p.base.speed, 0),
                    altitude_avg: p.base.altitude,
                    heart_rate_min: p.base.heart_rate,
                    heart_rate_max: p.base.heart_rate,
        ...
        let maxTime = pathData.last!.base.timestamp
        let interval = (maxTime - minTime) / 80.0
        var segments: [[RunningTrainingPathPoint]] = Array(repeating: [], count: 80)
        ...
        var samples: [RunningTrainingSamplePathPoint] = []
        for segment in segments {
            if segment.isEmpty {
                samples.append(RunningTrainingSamplePathPoint(
                    speed_avg: 0,
                    altitude_avg: 0,
                    heart_rate_min: nil,
                    heart_rate_max: nil,
        ...
        // speed_avg 异常值处理
        if samples.count >= 3 {
            var fixed: [RunningTrainingSamplePathPoint] = samples
            for i in 1..<(samples.count - 1) {
                let left = samples[i - 1].speed_avg
                let mid = samples[i].speed_avg
                let right = samples[i + 1].speed_avg
...
struct BikePathPointTool {
    ...
    static func computeTrainingSamplePoints(pathData: [BikeTrainingPathPoint]) -> [BikeTrainingSamplePathPoint] {
        guard !pathData.isEmpty else { return [] }
        if pathData.count <= 80 {
            // 当点数不超过80时，直接将每个点转换为区间相同的TestSamplePathPoint
            return pathData.map { p in
                BikeTrainingSamplePathPoint(
                    speed_avg: max(3.6 * p.base.speed, 0),
                    altitude_avg: p.base.altitude,
                    heart_rate_min: p.base.heart_rate,
                    heart_rate_max: p.base.heart_rate,
        ...
        let maxTime = pathData.last!.base.timestamp
        let interval = (maxTime - minTime) / 80.0
        var segments: [[BikeTrainingPathPoint]] = Array(repeating: [], count: 80)
        ...
        var samples: [BikeTrainingSamplePathPoint] = []
        for segment in segments {
            if segment.isEmpty {
                samples.append(BikeTrainingSamplePathPoint(
                    speed_avg: 0,
                    altitude_avg: 0,
                    heart_rate_min: nil,
                    heart_rate_max: nil,
        ...
        // speed_avg 异常值处理
        if samples.count >= 3 {
            var fixed: [BikeTrainingSamplePathPoint] = samples
            for i in 1..<(samples.count - 1) {
                let left = samples[i - 1].speed_avg
                let mid = samples[i].speed_avg
                let right = samples[i + 1].speed_avg
