...
struct RunningPathPointTool {
    // 采样处理比赛原始路径数据，便于展示
    static func computeRaceSamplePoints(pathData: [RunningPathPoint]) -> [RunningSamplePathPoint] {
    ...
    static func computeFreeTrainingSamplePoints(pathData: [RunningFreeTrainingPathPoint]) -> [RunningFreeTrainingSamplePathPoint] {
        guard !pathData.isEmpty else { return [] }
        if pathData.count <= 80 {
            // 当点数不超过80时，直接将每个点转换为区间相同的TestSamplePathPoint
            return pathData.map { p in
                RunningFreeTrainingSamplePathPoint(
                    speed_avg: max(3.6 * p.base.speed, 0),
                    altitude_avg: p.base.altitude,
                    heart_rate_min: p.base.heart_rate,
                    heart_rate_max: p.base.heart_rate,
        ...
        for segment in segments {
            if segment.isEmpty {
                samples.append(RunningFreeTrainingSamplePathPoint(
                    speed_avg: 0,
                    altitude_avg: 0,
                    heart_rate_min: nil,
                    heart_rate_max: nil,
        ...
        // speed_avg 异常值处理
        if samples.count >= 3 {
            var fixed: [RunningFreeTrainingSamplePathPoint] = samples
            for i in 1..<(samples.count - 1) {
                let left = samples[i - 1].speed_avg
                let mid = samples[i].speed_avg
                let right = samples[i + 1].speed_avg
    ...
    static func computeRouteTrainingSamplePoints(pathData: [RunningRouteTrainingPathPoint]) -> [RunningRouteTrainingSamplePathPoint] {
        guard !pathData.isEmpty else { return [] }
        if pathData.count <= 80 {
            // 当点数不超过80时，直接将每个点转换为区间相同的TestSamplePathPoint
            return pathData.map { p in
                RunningRouteTrainingSamplePathPoint(
                    speed_avg: max(3.6 * p.base.speed, 0),
                    altitude_avg: p.base.altitude,
                    heart_rate_min: p.base.heart_rate,
                    heart_rate_max: p.base.heart_rate,
        ...
        let maxTime = pathData.last!.base.timestamp
        let interval = (maxTime - minTime) / 80.0
        var segments: [[RunningRouteTrainingPathPoint]] = Array(repeating: [], count: 80)
        ...
        var samples: [RunningRouteTrainingSamplePathPoint] = []
        for segment in segments {
            if segment.isEmpty {
                samples.append(RunningRouteTrainingSamplePathPoint(
                    speed_avg: 0,
                    altitude_avg: 0,
                    heart_rate_min: nil,
                    heart_rate_max: nil,
        ...
        // speed_avg 异常值处理
        if samples.count >= 3 {
            var fixed: [RunningRouteTrainingSamplePathPoint] = samples
            for i in 1..<(samples.count - 1) {
                let left = samples[i - 1].speed_avg
                let mid = samples[i].speed_avg
                let right = samples[i + 1].speed_avg
...
struct BikePathPointTool {
    // 采样处理比赛原始路径数据，便于展示
    static func computeRaceSamplePoints(pathData: [BikePathPoint]) -> [BikeSamplePathPoint] {
        guard !pathData.isEmpty else { return [] }
        if pathData.count <= 80 {
            // 当点数不超过80时，直接将每个点转换为区间相同的TestSamplePathPoint
            return pathData.map { p in
    ...
    static func computeFreeTrainingSamplePoints(pathData: [BikeFreeTrainingPathPoint]) -> [BikeFreeTrainingSamplePathPoint] {
        guard !pathData.isEmpty else { return [] }
        if pathData.count <= 80 {
            // 当点数不超过80时，直接将每个点转换为区间相同的TestSamplePathPoint
            return pathData.map { p in
                BikeFreeTrainingSamplePathPoint(
                    speed_avg: max(3.6 * p.base.speed, 0),
                    altitude_avg: p.base.altitude,
                    heart_rate_min: p.base.heart_rate,
                    heart_rate_max: p.base.heart_rate,
        ...
        for segment in segments {
            if segment.isEmpty {
                samples.append(BikeFreeTrainingSamplePathPoint(
                    speed_avg: 0,
                    altitude_avg: 0,
                    heart_rate_min: nil,
                    heart_rate_max: nil,
        ...
        // speed_avg 异常值处理
        if samples.count >= 3 {
            var fixed: [BikeFreeTrainingSamplePathPoint] = samples
            for i in 1..<(samples.count - 1) {
                let left = samples[i - 1].speed_avg
                let mid = samples[i].speed_avg
                let right = samples[i + 1].speed_avg
    ...
    static func computeRouteTrainingSamplePoints(pathData: [BikeRouteTrainingPathPoint]) -> [BikeRouteTrainingSamplePathPoint] {
        guard !pathData.isEmpty else { return [] }
        if pathData.count <= 80 {
            // 当点数不超过80时，直接将每个点转换为区间相同的TestSamplePathPoint
            return pathData.map { p in
                BikeRouteTrainingSamplePathPoint(
                    speed_avg: max(3.6 * p.base.speed, 0),
                    altitude_avg: p.base.altitude,
                    heart_rate_min: p.base.heart_rate,
                    heart_rate_max: p.base.heart_rate,
        ...
        let maxTime = pathData.last!.base.timestamp
        let interval = (maxTime - minTime) / 80.0
        var segments: [[BikeRouteTrainingPathPoint]] = Array(repeating: [], count: 80)
        ...
        var samples: [BikeRouteTrainingSamplePathPoint] = []
        for segment in segments {
            if segment.isEmpty {
                samples.append(BikeRouteTrainingSamplePathPoint(
                    speed_avg: 0,
                    altitude_avg: 0,
                    heart_rate_min: nil,
                    heart_rate_max: nil,
        ...
        // speed_avg 异常值处理
        if samples.count >= 3 {
            var fixed: [BikeRouteTrainingSamplePathPoint] = samples
            for i in 1..<(samples.count - 1) {
                let left = samples[i - 1].speed_avg
                let mid = samples[i].speed_avg
                let right = samples[i + 1].speed_avg
