        ...
        ZStack(alignment: .bottom) {
                            ...
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                LazyVGrid(columns: [
                                    ...
                                    InfoItemView(
                                        iconName: "person.2",
                                        iconColor: .purple,
                                        text: "当前参与人数: \(track.currentParticipants)"
                                    )
                                    // 路程
                                    InfoItemView(
                                        iconName: "road.lanes.curved.right",
                                        iconColor: .gray,
                                        text: "路程: \(track.distance)km"
                                    )
                                }
                                .padding(.vertical, 6)
                                ...
                                HStack(spacing: 12) {
                                    ...
                                    .sheet(isPresented: $viewModel.showCreateTeamSheet) {
                                        RunningTeamCreateView(viewModel: viewModel)
                                    }
                                    ...
                                    .sheet(isPresented: $viewModel.showJoinTeamSheet) {
                                        RunningTeamJoinView(viewModel: viewModel)
                                    }
                                }
                            }
                            .padding()
            ...
            HStack(spacing: 0) {
                ...
                .sheet(isPresented: $viewModel.showTeamCodeSheet) {
                    RunningTeamCodeView(viewModel: viewModel)
...
// 赛道信息项组件
struct RunningInfoItemView: View {
    let iconName: String
    let iconColor: Color
    let text: String
...
// 组队报名页面
struct RunningTeamCodeView: View {
    @EnvironmentObject var appState: AppState
    //@ObservedObject var centerViewModel: SportCenterViewModel
    @ObservedObject var viewModel: RunningCompetitionViewModel
    @ObservedObject var currencyManager = CurrencyManager.shared
