//
//  BikeRaceRecordManagementView.swift
//  sportsx
//
...
struct BikeRaceRecordManagementView: View {
    @EnvironmentObject var appState: AppState
    @StateObject var viewModel = BikeRaceRecordManagementViewModel()
            ...
            if record.status == .completed || record.status == .expired || record.status == .invalid {
                HStack {
                    ...
                    CommonTextButton(text: "详情") {
                        appState.navigationManager.append(.bikeRecordDetailView(recordID: record.record_id))
                    }
                    .font(.subheadline)
                    .padding(.horizontal, 12)
                    .padding(.vertical, 6)
...
#Preview {
    ...
    return BikeRaceRecordManagementView()
