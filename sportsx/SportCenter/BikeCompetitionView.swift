...
struct BikeCompetitionView: View {
                        ...
                        if let event = viewModel.selectedEvent {
                            VStack(alignment: .leading, spacing: 10) {
                                HStack(alignment: .top) {
                                    ...
                                    .foregroundColor(.secondText)
                                    .exclusiveTouchTapGesture {
                                        appState.navigationManager.append(.bikeEventDetailView(eventID: event.eventID))
                                    }
                                }
    ...
    func singleRegister() {
        ...
        NetworkService.sendRequest(with: request, decodingType: BikeRegisterResponse.self, showLoadingToast: true, showErrorToast: true) { result in
            switch result {
...
struct BikeEventDetailView: View {
                        ...
                        VStack(alignment: .leading, spacing: 16) {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                Text("competition.event.precautions")
                                    .font(.headline)
                                    .foregroundStyle(Color.white)
                                Text("competition.event.bike.precautions.content")
                                    .font(.subheadline)
                                    .foregroundStyle(Color.secondText)
                            }
                        }
    ...
    func queryEventDetail() {
        guard var components = URLComponents(string: "/competition/bike/query_event_detail") else { return }
        components.queryItems = [
        ...
        NetworkService.sendRequest(with: request, decodingType: BikeEventInfoDTO.self, showLoadingToast: true, showErrorToast: true) { result in
            switch result {
            case .success(let data):
                if let unwrappedData = data {
                    DispatchQueue.main.async {
                        event = BikeEvent(from: unwrappedData)
                    }
