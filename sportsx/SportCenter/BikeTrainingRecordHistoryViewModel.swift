//
//  BikeTrainingRecordHistoryViewModel.swift
//  sportsx
//
...
@MainActor
class BikeTrainingRecordHistoryViewModel: ObservableObject {
    @Published var calendarDays: [CalendarDay] = []
    @Published var records: [BikeFreeTrainingRecord] = []
    ...
    private var monthHistoryCache: [String: [BikeTrainingStatesHistoryInfo]] = [:]
    private var recordsCache: [String: [BikeFreeTrainingRecord]] = [:]
    ...
    private func fetchHistory() async {
        ...
        guard var components = URLComponents(string: "/training/bike/training_states/month") else { return }
        ...
        let result = await NetworkService.sendAsyncRequest(with: request, decodingType: BikeTrainingStatesHistoryResponse.self, showLoadingToast: true, showErrorToast: true)
        switch result {
    ...
    private func fetchRecords(for date: Date) {
        ...
        guard var components = URLComponents(string: "/training/bike/training_records/day") else { return }
        components.queryItems = [
        ...
        NetworkService.sendRequest(with: request, decodingType: TrainingRecordsResponse.self, showLoadingToast: true, showErrorToast: true) { result in
            ...
            case .success(let data):
                if let unwrappedData = data {
                    DispatchQueue.main.async {
                        let newRecords = unwrappedData.records.map { BikeFreeTrainingRecord(from: $0) }
                        // 存缓存
...
struct BikeTrainingStatesHistoryResponse: Codable {
    let history: [BikeTrainingStatesHistoryInfo]
}
