//
//  RunningTeamManagementView.swift
//  sportsx
//
...
struct RunningTeamManagementView: View {
    @EnvironmentObject var appState: AppState
    @StateObject var viewModel = RunningTeamManagementViewModel()
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                (Text(LocalizedStringKey(SportName.Running.name)) + Text("competition.team.manage"))
                    ...
                    if viewModel.myCreatedTeams.isEmpty {
                        ...
                        LazyVStack(spacing: 15) {
                            ForEach(viewModel.myCreatedTeams) { team in
                                RunningTeamCardView(viewModel: viewModel, type: .created, team: team)
                                    .onAppear {
                    ...
                    if viewModel.myAppliedTeams.isEmpty {
                        ...
                        LazyVStack(spacing: 15) {
                            ForEach(viewModel.myAppliedTeams) { team in
                                RunningTeamAppliedCardView(viewModel: viewModel, team: team)
                                    .onAppear {
                    ...
                    if viewModel.myJoinedTeams.isEmpty {
                        ...
                        LazyVStack(spacing: 15) {
                            ForEach(viewModel.myJoinedTeams) { team in
                                RunningTeamCardView(viewModel: viewModel, type: .joined, team: team)
                                    .onAppear {
...
struct RunningTeamAppliedCardView: View {
    @EnvironmentObject var appState: AppState
    @ObservedObject var viewModel: RunningTeamManagementViewModel
...
struct RunningTeamCardView: View {
    @EnvironmentObject var appState: AppState
    @ObservedObject var viewModel: RunningTeamManagementViewModel
            ...
            HStack {
                ...
                if type == .created {
                    ...
                    CommonTextButton(text: "competition.team.action.manage") {
                        appState.navigationManager.append(.runningTeamManageView(teamID: team.team_id))
                    }
                    .font(.caption)
                    .fontWeight(.medium)
                    .foregroundStyle(Color.white)
                    ...
                    CommonTextButton(text: "action.detail") {
                        appState.navigationManager.append(.runningTeamDetailView(teamID: team.team_id))
                    }
                    .font(.caption)
                    .fontWeight(.medium)
                    .foregroundStyle(Color.white)
    ...
    // 退出队伍
    func exitTeam() {
        guard var components = URLComponents(string: "/competition/running/quit_team") else { return }
    ...
    // 解散队伍
    func disbandTeam() {
        guard var components = URLComponents(string: "/competition/running/disband_team") else { return }
        components.queryItems = [
            URLQueryItem(name: "team_id", value: team.team_id)
        ]
        guard let urlPath = components.string else { return }
...
struct RunningTeamDetailView: View {
    @EnvironmentObject var appState: AppState
    @StateObject var viewModel = RunningTeamDetailViewModel()
    let teamID: String
...
// 队伍管理视图（用于已创建的队伍）
struct RunningTeamManageView: View {
    @EnvironmentObject var appState: AppState
    @StateObject var viewModel: RunningTeamManageViewModel
    
    init(teamID: String) {
        _viewModel = StateObject(wrappedValue: RunningTeamManageViewModel(teamID: teamID))
    }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $viewModel.showAppliedDetail) {
            RunningAppliedDetailView(viewModel: viewModel)
                .presentationDetents([.fraction(0.4)])
                .interactiveDismissDisabled()
        }
        .bottomSheet(isPresented: $viewModel.showTeamEditor, size: .large) {
            RunningTeamEditorView(viewModel: viewModel)
        }
...
struct RunningAppliedDetailView: View {
    @ObservedObject var viewModel: RunningTeamManageViewModel
...
struct RunningTeamEditorView: View {
    @ObservedObject var viewModel: RunningTeamManageViewModel
