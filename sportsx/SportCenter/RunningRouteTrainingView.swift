    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            HStack {
                ...
                if !route.isPublic {
                    Button(action: {
                        appState.navigationManager.append(.runningRouteEditView(route: route))
                    }) {
                       Text("action.edit")
                            .font(.system(size: 15))
                            .foregroundStyle(Color.white)
...
struct RunningRouteManageView: View {
    ...
    @State private var applyingRoute: RunningRouteManageItem? = nil
    let pageSize: Int = 10
    ...
    var body: some View {
        ...
        .bottomSheet(isPresented: $showApplyForm, size: .large, destroyOnDismiss: true) {
            if let route = applyingRoute {
                RunningRouteApplyFormView(route: route, isPresented: $showApplyForm) {
...
struct RunningRouteManageItem: Identifiable, Hashable {
    ...
    static func == (lhs: RunningRouteManageItem, rhs: RunningRouteManageItem) -> Bool {
        lhs.id == rhs.id
    }
            ...
            ScrollView {
                VStack(spacing: 15) {
                    ...
                    HStack {
                        ...
                        Spacer()
                        CapsuleScrollSelector(options: RunningTrackTerrainType.allCases, selection: $terrainType) { $0.displayName }
                    }
...
// 路线编辑页：结构与 RunningRouteCreateView 基本一致，底部按钮由"创建"调整为"修改"逻辑
struct RunningRouteEditView: View {
    let route: RunningRouteManageItem
    @StateObject var store: RouteEditorStore = RouteEditorStore()
    @ObservedObject var navigationManager = NavigationManager.shared
    @ObservedObject var locationManager = LocationManager.shared
    ...
    func updateRoute() {
        ...
        let request = APIRequest(path: "/training/running/routes/update", method: .post, headers: headers, body: encodedBody, requiresAuth: true)
...
// 申请热门路线转为赛道的表单
struct RunningRouteApplyFormView: View {
    ...
    @State private var terrainType: RunningTrackTerrainType
    @State private var lifecycle: TrackLifecycle = .oneMonth
    @State private var isPremium: Bool
    @State private var isSubmitting: Bool = false

    init(route: RunningRouteManageItem, isPresented: Binding<Bool>, onSuccess: @escaping () -> Void) {
        self.route = route
    ...
    private func submit() {
        ...
        let request = APIRequest(path: "/training/running/routes/apply_track", method: .post, headers: headers, body: encodedBody, requiresAuth: true)
        NetworkService.sendRequest(with: request, decodingType: EmptyResponse.self, showLoadingToast: true, showErrorToast: true) { result in
            DispatchQueue.main.async {
                isSubmitting = false
                switch result {
