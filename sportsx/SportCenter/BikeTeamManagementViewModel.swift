...
class BikeTeamManagementViewModel: ObservableObject {
    ...
    func queryAppliedTeams(withLoadingToast: Bool, reset: Bool) async {
        ...
        case .success(let data):
            if let unwrappedData = data {
                var tempTeams: [BikeTeamAppliedCard] = []
                for team in unwrappedData.teams {
                    tempTeams.append(BikeTeamAppliedCard(from: team))
                }
                if reset {
    ...
    func queryCreatedTeams(withLoadingToast: Bool, reset: Bool) async {
        ...
        case .success(let data):
            if let unwrappedData = data {
                var tempTeams: [BikeTeamCard] = []
                for team in unwrappedData.teams {
                    tempTeams.append(BikeTeamCard(from: team))
                }
                if reset {
...
class BikeTeamManageViewModel: ObservableObject {
    ...
    func saveTeamInfo() {
        ...
        NetworkService.sendRequest(with: request, decodingType: BikeTeamUpdateResponse.self, showLoadingToast: true, showErrorToast: true) { result in
            switch result {
            case .success(let data):
    ...
    func removeMember(with memberID: String) {
        ...
        NetworkService.sendRequest(with: request, decodingType: BikeTeamMemberUpdateResponse.self, showLoadingToast: true, showErrorToast: true) { result in
            switch result {
            case .success(let data):
                self.globalConfig.refreshTeamManageView = true
                if let unwrappedData = data {
